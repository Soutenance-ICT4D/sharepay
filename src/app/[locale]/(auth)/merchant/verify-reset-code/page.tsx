...
export default function VerifyResetCodePage() {
    ...
    async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const response = await authService.verifyResetCode({ email, otp: code });
            toast.success(tGlobal(`Auth.Success.${SUCCESS_KEYS.AUTH_VERIFY_RESET_CODE}`));
            router.push(`/merchant/reset-password?email=${encodeURIComponent(email)}&token=${encodeURIComponent(response.resetToken)}`);
        } catch (error: any) {
            const { messageKey, values } = resolveError(error);
            toast.error(tGlobal(messageKey, values));
        } finally {

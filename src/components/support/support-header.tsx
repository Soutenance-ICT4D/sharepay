...
import { Menu, ChevronRight, Settings, LogOut, Headphones } from "lucide-react";
...
export function SupportHeader({ onMenuClick }: SupportHeaderProps) {
    const t = useTranslations("Support.Header");
    const tb = useTranslations("Support");
    ...
    const [user] = useState(() => {
        ...
        return {
            name: info?.fullName ?? "Support",
            email: info?.email ?? "",
        };
    });

    const getInitials = (name: string) =>
        name.split(" ").filter(Boolean).slice(0, 2).map((n) => n[0]).join("").toUpperCase() || "S";
        ...
        <div className="sticky top-0 z-20">
            <header className="h-16 border-b bg-background/80 backdrop-blur-md px-6 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    {/* Notifications */}
                    <Button variant="ghost" size="icon" className="relative" onClick={() => router.push("/support/notifications")}>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="rounded-full" suppressHydrationWarning>
                                <Avatar className="h-8 w-8 border border-blue-500/20 hover:border-blue-500/40 transition-colors">
                                    <AvatarFallback className="bg-blue-500/10 text-blue-600 text-sm font-bold">
                        ...
                        <DropdownMenuContent align="end" className="w-52">
                            <DropdownMenuLabel className="font-semibold">
                                <p className="flex items-center gap-1.5">
                                    <Headphones className="h-3.5 w-3.5 text-blue-600 shrink-0" />
                                    <span className="truncate">{user.name}</span>
                                </p>
                                <p className="text-xs text-muted-foreground font-normal truncate">{user.email}</p>
                            </DropdownMenuLabel>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem onClick={() => router.push("/support/settings")}>

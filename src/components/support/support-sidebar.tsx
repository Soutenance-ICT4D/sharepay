...
export function SupportSidebar({ isOpen, isCollapsed, onClose, onToggleCollapse }: SupportSidebarProps) {
    const t = useTranslations('Support.Sidebar');
            ...
            `}>
                <div className="h-16 flex items-center px-6 border-b bg-blue-500/5">
                    ...
                    >
                        ...
                        {!isCollapsed && <span className="whitespace-nowrap transition-opacity duration-300 text-blue-600">Support</span>}
                    </Link>
                </div>
                ...
                <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
                    {navItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                <span className={`h-10 w-10 min-w-[40px] rounded-lg flex items-center justify-center shrink-0 transition-colors ${isActive ? "bg-blue-500/10" : "bg-muted/40 group-hover:bg-muted/60"}`}>
                                    {item.icon}
                                </span>
